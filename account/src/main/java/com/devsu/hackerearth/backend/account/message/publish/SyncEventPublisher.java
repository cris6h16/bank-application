...
public class SyncEventPublisher implements EventPublisher { // here i should use kfafk template, but to make this simple
    ...
    public void publish(OutboxEvent event) {
        ...
        if (Objects.equals(event.getAggregateType(), AggregateTypeEnum.CLIENT)) { // requests to client service (:8001)
            ResponseEntity<Void> response = restTemplate.postForEntity("http://localhost:8001/api/events", dto,
                    Void.class);
